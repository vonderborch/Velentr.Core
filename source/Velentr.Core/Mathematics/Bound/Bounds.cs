...
namespace Velentr.Core.Mathematics.Bound
{
    ...
    /// </summary>
    /// <typeparam name="T">The type of the values, which must implement INumber&lt;T&gt;.</typeparam>
    ...
    public struct Bounds<T> : IEquatable<Bounds<T>> where T : INumber<T>
    {
        ...
        {
            return EqualityComparer<T>.Default.Equals(this.maximum, other.maximum) && EqualityComparer<T>.Default.Equals(this.minimum, other.minimum);
        }
    }
}
